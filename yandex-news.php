...
class YandexNews
{
    ...
    public function __construct()
    {
        ...
        $this->feedname = ( !empty( $this->options['path'] ) ) ? $this->options['path'] : 'yandex';
    ...
    public function add_feed()
    {
        if ( empty( $this->options['logo'] ) ) {
            if ( !empty( $this->options['image'] ) ) {
                $this->options['logo'] = $this->options['image'];
            }
        }
        
        if ( empty( $this->options['logo_square'] ) ) {
            if ( !empty( $this->options['image'] ) ) {
                $this->options['logo_square'] = $this->options['image'];
            }
        }

        add_feed($this->feedname, array( $this, 'do_feed') );
    }
    ...
    {        
        ...
        add_settings_field(
            'logo_square',
            __( 'Logo square', 'yandex-news' ),
            array( $this, 'logo_square_callback' ),
            'yandex-news-admin',
            'yandex_news_feed_settings'
        );
    ...
    public function sanitize( $input )
    {
        ...
        if ( strlen( $new_input['logo'] ) == 0 ) {
            add_settings_error(
              'logo',
              esc_attr( 'settings_updated' ),
              __( 'Specify a value into the Logo field', 'yandex-news' )
            );
        }

        $new_input['logo_square'] = esc_url_raw( $input['logo_square'] );
        if ( strlen( $new_input['logo_square'] ) == 0 ) {
            add_settings_error(
              'logo_square',
              esc_attr( 'settings_updated' ),
              __( 'Specify a value into the Logo square field', 'yandex-news' )
            );
        }
    ...
    {
        if ( $this->options['logo'] && !get_settings_errors( 'logo' ) ) {
            ...
            'yandex-news' ) . '</p>', get_feed_link($this->feedname) );
        }
        else {
            settings_errors( 'logo', true, true );
    ...
    {
        printf(
            '<input class="regular-text" type="text" id="logo"
            name="yandex_news[logo]" value="%s" /><p class="description">' .
            __('Logo', 'yandex-news') . ' (' .
            __('required field', 'yandex-news') . ')</p>', 
            isset( $this->options['logo'] ) ? esc_attr(
            $this->options['logo'] ) : '' );
    }
    
    public function logo_square_callback()
    {
        printf(
            '<input class="regular-text" type="text" id="logo_square"
            name="yandex_news[logo_square]" value="%s" /><p class="description">' .
            __('Logo square', 'yandex-news') . ' (' .
            __('required field', 'yandex-news') . ')</p>', 
            isset( $this->options['logo_square'] ) ? esc_attr(
            $this->options['logo_square'] ) : '' );
    }
